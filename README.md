...
- **Core Focus**: 
  - **High-Performance Systems**: 대규모 데이터 처리와 지연(Latency) 최소화를 위한 고성능 아키텍처 설계
  - **Compiler Tooling**: LLVM/Clang 기반의 정적 분석(Static Analysis) 엔진 및 도구 체인 구축
  - **Concurrent Architecture**: 동시성 제어 및 논블로킹(Non-blocking) 기반의 확장 가능한 서버/앱 아키텍처
  - **AI Orchestration**: 분산 시스템 환경에서의 비용 최적화된 자율 AI 에이전트 오케스트레이션
...
### Languages
<img src="https://img.shields.io/badge/C%2B%2B20-00599C?style=flat-square&logo=c%2B%2B&logoColor=white" /> <img src="https://img.shields.io/badge/C%23%2012.0-239120?style=flat-square&logo=c-sharp&logoColor=white" />
...
- **Profiling & Optimization**: Tracy Profiler를 연동하여 ECS 틱 루프와 스레드 병목 지점을 실시간 계측. 데이터 지향 설계(DOD) 적용으로 OOP 대비 CPU 캐시 히트 성능 4.2배 향상, 비동기 큐의 더블 버퍼링 도입으로 극한 경합 상태에서의 락(Lock) 점유 지연을 35배 억제하여 안정적인 20Hz 틱레이트를 방어.
---
